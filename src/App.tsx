...
import {
  createEffect, createMemo, from, onCleanup, Show,
} from 'solid-js';

import styles from './App.module.css';
import {
  $comletedTasks, $existingTasks, $tasks, addTask, addTasks, editTask, removeTask,
} from './models/tasks/init';
...
const App: Component = () => {
  ...
  const successPercent = createMemo(() => (totalTasks() !== 0
    ...
    <div class={styles.App}>
      <header class={styles.header}>
        ...
        <div class={styles.headerSubLine}>
            ...
            {<Show when={successPercent() !== null} fallback={undefined}>{<ChartLine value={successPercent() || 0} title={`${completedTasks().length}/${totalTasks()}`} />}</Show>}
        </div>
